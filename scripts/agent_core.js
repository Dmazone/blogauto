...
async function writeArticle(section, topic, outline, validated) {
  ...
  const body = await geminiCall(
    ...
    `- 해시태그 바로 위: ## 자주 묻는 질문 섹션 — 독자가 실제로 검색할 만한 Q&A 3개 (### Q: ... \\n A: ... 형식)\n` +
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔴 최우선 원칙: 오늘(${todayKst}) 기준 실시간 최신 이슈만 다룬다
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

구글 검색으로 이 섹션에서 오늘 날짜 기준 가장 화제가 되는 트렌드 주제를 3개 조사해줘.
각 주제별로:
① 이 이슈가 뜨거워진 날짜: YYYY-MM-DD (반드시 명시 — 날짜 모르거나 2주 이상 전이면 탈락)
② 왜 지금 핫한지 — 최근 72시간 내 구체적 사건·보도·발언 기반으로 설명
③ 실제로 구글에서 검색할 법한 검색어 예시 2개
④ 구글 애드센스 노출 가능성 평가
⑤ 추천 제목 방향: 의문형("~인가?") / 숫자형("3가지", "Best 5") / 반전형("~라면 틀렸다") 중 1개

🚫 주제 자동 탈락 조건 (하나라도 해당하면 즉시 제외):
- 이슈 발생일이 2주 이상 전이고 새 전개·속보·반전이 없는 경우
- "경제 트렌드", "최신 기술" 같은 범용 정보성 주제 (구체적 인물·기업·사건·날짜 없음)
- 학습 데이터에만 있고 오늘 실제 구글 검색 결과에 없는 주제
- 이미 결론 난 과거 이슈 (예: 수개월 전에 피크를 찍고 지금은 반전된 주제)
...
해시태그 바로 위 줄에 반드시 아래 고지 문구를 넣어라:

---

> 이 포스팅은 쿠팡 파트너스 활동의 일환으로, 이에 따른 일정액의 수수료를 제공받습니다.

마지막 줄: 한국어 해시태그 7개 이상 (#트렌드상품 #쿠팡추천 #${topic.keyword} ...)`
  );
