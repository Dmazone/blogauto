...
### 8. 내부 링크 검증 — 저장 전 슬러그 존재 확인 ★
- 포스팅 마지막 H2에 **내부 링크 1개 이상** 필수 (SEO + 체류시간)
- 링크 형식: `[앵커텍스트](/posts/섹션/slug/)` — 절대 경로로 작성 (config.toml의 `canonifyURLs=true`가 빌드 시 자동으로 전체 URL로 변환)
- **저장 전 반드시 링크 대상 슬러그 실존 여부 확인**:
  ```bash
  # 슬러그가 존재하는지 확인
  ls content/posts/섹션/slug/index.md
  ```
  - 파일이 없으면 해당 슬러그 링크 사용 금지
  - 대신 섹션 목록 링크(`/posts/섹션/`)로 대체
- 같은 섹션 내 포스팅 우선 연결, 없으면 연관 섹션 포스팅 연결
- **존재하지 않는 슬러그로 링크하면 404 에러** → 반드시 확인 후 저장
...
- **내부 링크 슬러그 실존 확인**: `ls content/posts/섹션/slug/index.md` 로 대상 파일 존재 여부 검증 → 없으면 섹션 링크(`/posts/섹션/`)로 교체 후 저장
